...
def get_province(str):
    flag = re.search(("(.*?省)|(.*?自治区)|上海市|北京市|天津市|重庆市"), s)
    if flag != None:
        length = len(flag.group())
    if flag == None or length>= 6:
        if str[0:3] == "黑龙江":
            return "黑龙江"
        if str[0:3] == "内蒙古":
            return "内蒙古"
...
def get_city(str):
    flag = re.search("(.*?自治州)|(.*?[市])", str)
    if flag != None:
        length = len(flag.group())
    if flag == None or length >=8:
        for i in cities:
            if str[0:2] in i:
                return i
        return ""
...
while(1):
    ...
    name = result[0]
    str = result[1]
    telephone = re.search(r'\d{11}', string)
    str = str.replace(telephone, "", 1)
    str = str.replace(".", "", 1)
